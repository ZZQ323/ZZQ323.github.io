...
在 模m 体系中，对于任意一个整数x，我都能在m个整数 $a_1,a_2,\dots,a_n$ 中找到对应的逆元，那么这m个数就叫做（完全）剩余系。

对于所有的非零整数，把这么多数字不相交地分成m个集合，其中mod m 同余的数字放一起，那么这m个集合就都叫做同余类。

对于m个同余类中的一个同余类$r\mod m$，如果r和m互素，那么这个同余类就是既约同余类、既然约剩余类；

既约剩余类的个数记作$\varphi(m)$，也就是欧拉函数。
